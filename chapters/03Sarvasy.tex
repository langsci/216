...
The \ili{Bantu} chains generally differ from those in \ili{Nungon} and \isi{clause chaining} languages of most other families in two ways. First, subject person and number are obligatorily marked on all clausal predicates within the \ili{Bantu} chains. Second, as noted by \citet{Haspelmath.1995} and others for \ili{Swahili}, in clause chains in \ili{Bantu} languages it is the predicate of the first clause that is \isi{finite} (marked for tense), rather than the last, as in the \ili{Nungon} example above. If \ili{Bantu} languages have bridging constructions, then, they could pose a challenge for \citeauthor{guerin18} (this volume)’s assumption that bridging clauses are ``non-main'' and reference clauses are ``main''. If the bridging clause begins a new \isi{clause chain} and the reference clause ends the preceding \isi{clause chain}, \ili{Bantu} patterns predict that the bridging clause should be \isi{finite} and the reference clause, if it ends a \isi{clause chain}, should be non-\isi{finite}. But in accordance with \citeauthor{guerin18}’s summary, \ili{Logoori} bridging clauses -- albeit \isi{finite} -- are prosodically and semantically dependent, while non-\isi{finite} reference clauses are prosodically and semantically main clauses (see \refsec{Sa21form}). 
...
1\textsc{sg}-\textsc{imm.pf-}pick.leaves.from.stems-\textsc{fv}  \textsc{narr}-\textsc{1sg}-lay.out.to.dry-\textsc{fv}\\
...
\gll \textbf{Gw-aka-ɲar-a}...\\     	       
    3-\textsc{imm.pf-}shrivel-\textsc{fv}\\
...
\textsc{1sg-imm.pf}-gather.up-\textsc{fv}  5-ash\\
\glt \sqt{I’ve just gathered up ash,}\\
\ex \label{Saex:5b}
\gll n-daka-vunaɲer-a     zi-ŋgu     jemo,\\
\textsc{1sg-imm.pf-}break-\textsc{fv}  10-wood  in.here\\
...
\gll n-daka-vogor-a   ke-biridi,\\     	       
   \textsc{1sg-imm.pf-}take-\textsc{fv}  7-match\\
...
\lsptoprule
                                    & \textbf{Reference clause} & \textbf{Bridging clause}       \\
\midrule
\textbf{Tense}     & Narrative, or             & Immediate Perfect              \\
                                    & periphrastic Near Future  &                                \\    \cline{1-3} 
\textbf{Subject person/number}      & Free                      & As in the reference clause     \\ \cline{1-3}                 
\textbf{Semantics} &    	Introduction of					  & Close temporal link between      \\
                                    & a new \isi{action}                    & \isi{action} in the reference  \\
                                    &                            & clause and \isi{action} in the    \\
                                    &                           & succeeding clause                        \\ \cline{1-3}
 \textbf{Prosody}                    & Final                     & Non-final                      \\
\lspbottomrule
\end{tabular}
...
\exi{(A8)} \label{Saapp8}
\gll \textbf{N-daka-n̪ɔr-a},          \underline{\smash{a-m-bagar-e}}.\\
1\textsc{sg}-\textsc{imm.pf-}pick.leaves.from.stems-\textsc{fv}  \textsc{narr-1sg}-lay.out.to.dry-\textsc{fv}\\
\glt \sqt{Once I have picked the leaves from the stems, I lay them out to dry.}
\end{exe}
...
\textsc{1sg}-\textsc{imm.pf}-finish-\textsc{fv} 3-\textsc{past}?-leach-\textsc{appl}-\textsc{fv}  3.soup  \\
