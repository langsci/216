...
\begin{exe}
	\ex	\label{Saex:2}
\langinfo{Bantu verb inflection slots}{}{\citealt[][90]{Nurse2003}}
...
Two other instances of lexical repetition that diverge from the bridging construction pattern in \refsec{Sa21form} come from a fairy tale told by Ms. Grace Otieno. Here, two clauses that begin new prosodic sentences feature lexical repetition of predicates from the preceding sentences. But in both cases, the word \textit{ruwa} `while, when' precedes the recapitulated verb. This would seem to be a different type of linkage than the simple recapitulation in \refsec{Sa21form}. One of these examples is in (\ref{Saex:8ab}):
