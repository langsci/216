...
In this example, the subject of the medial clause in (\ref{Aiex:03a}) is \textit{nɛ ɛjaja} `my father', who is described as being at a location, whereas in the \isi{final clause} (\ref{Aiex:03b}) the subject is the \isi{coordinated} noun phrase \textit{saːgoi ɛjalɛ motuwɛ ɛjalɛ} `Saːgai and Motuwe', who are coming while hitting trees. Clauses and noun phrases may additionally be morphologically topicalized, in which case, different subject marking functions in much the same way as in example (\ref{Aiex:03ab}), as can be seen in (\ref{Aiex:04ac}) where the verb in the \isi{topic} clause is suffixed by \textit{-bi} since its subject differs from that of the main predicate.
...
\isi{clause chain} boundaries featuring verbatim \isi{repetition} of clauses, and larger paragraphs being bounded
...
in examples (\ref{Aiex:App23}--\ref{Aiex:App24}) of the appendix, the first medial \isi{non-main clause} includes the predicate \textit{hɛnaː} \textit{disi}, which is not specified for
...
Every line given in (\ref{Aiex:06af}) is a \isi{clause chain}, and the main clauses (\ref{Aiex:06c} to \ref{Aiex:06f}) each begin with a non-main medial
...
The current discussion will not further discuss topical noun phrases, and will focus on the occurrence of clauses as the \isi{topic} of a subsequent \isi{main clause}.
...
In this construction, the clause linking suffix \textit{-si} provides aspectual information regarding the timing of the \isi{topic} with respect to the \isi{main clause}. Specifically, the \isi{topic} and \isi{main clause} are consecutive events, where the \isi{topic} clause is a perfective event occurring immediately prior to the \isi{main clause}.
	In \isi{addition} to these semantic and functional considerations, topical clauses containing an auxiliary within the predicate require the clause linking suffix  \textit{-si}. This is true even if the aspectual information provided by the suffix  \textit{-si} is redundant as in (\ref{Aiex:11b}).
...
\label{Ailightverb.do}
	The \isi{light verb} \textit{ɛ} is by far the most common \isi{summary linkage} strategy. It does not occur with any tense morphology, but a variety of aspectual and conjunctive enclitics, including \isi{switch reference}, perfectivity, and completion. The reference of \textit{ɛ} `do' is non-specific and general. In (\ref{Aiex:App29ab}) of the appendix, the \isi{topic} clause \textit{ɛbija} ‘do’ makes reference to the preceding \isi{final clause}, \textit{ɛimɛ oːɸa aːnɛ} `The sun set'.
