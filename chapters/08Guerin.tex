...
In this section, I review the formal properties of \isi{recapitulative linkage} in \ili{Mavea}. Questions addressed touch on the composition and content of the bridging clause (what is repeated and how) and the status of the bridging clause (whether a main or \isi{non-main clause}, a final or a \isi{non-final clause}), in \refsec{Gusec:Status}.
...
\glt \sqt{He got scared and he started to run, and he (n.t.: someone else) said: ``Hey! Don't run!''} 
\end{xlist}
\end{exe}
...
\glt \sqt{He pulled (it) down to the sea shore.}\\
\ex \label{Guex:06b}
...
\glt \sqt{He pulled the banana-log raft down to the sea shore, he put it to float.} 
\end{xlist}
\end{exe}
...
In \ili{Mavea}, on the other hand, bridging clauses do not show any sign of grammatical dependency. They are not restricted in their inflectional possibilities: they show no limitations on the tense, reality status, mood/modality, etc., that they can mark. They can be negated, as shown in (\ref{Guex:01ab}); they show no restriction on the presence or absence of core arguments. In \isi{addition}, the bridging clause is often \isi{coordinated} to the following clause with the coordinator \textit{ro} `and, and then, then', as shown in (\ref{Guex:03ab}). This coordinator conjoins verb phrases and clauses, shown in (\ref{Guex:10}), but not nominals \citep[][314ff]{guerin11}.\footnote{Note in passing that the \isi{conjunction} \textit{ro} generally forms an intonational unit with the first conjunct or with the bridging clause and is followed by a pause \citep[][321]{guerin11}.} Thus, in all morpho-syntactic aspects, bridging clauses are just like any other main clauses: they do not constitute a separate \isi{clause type}.  
...
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig1.eps}}
...
\glt \sqt{the hat goes down on the ground.} 
\end{xlist}
\end{exe}

\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig2.eps}}
\caption{Intonation contour of examples (\ref{Guex:12a}) and (\ref{Guex:12b}). \label{GuF2}}
\end{figure}


Example (\ref{Guex:13ac}) also contains a \isi{recapitulative linkage}, but this time, the bridging clause (\ref{Guex:13b}) is overtly \isi{coordinated} to the following clause. All three clauses are represented in Fig.~\ref{GuF3}. 
...
\begin{xlist}
\ex \label{Guex:14a}
\gll \underline{\smash{Ko-pos}}          \underline{ko-si}                 \underline{ko-sev!}     [1.06s]\\
...
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig4.eps}}
\caption{Intonation contour of example (\ref{Guex:14ac}). \label{GuF4}}
\end{figure}

Needless to say, a rising tune is not specific to bridging clauses. Time adverbial clauses have a similar \isi{intonation} contour, as shown in Fig.\ref{GuF5}, since they too indicate continuation. Other clauses which are considered part of a chain of thought and thus non-final will also have a rising \isi{intonation} contour, regardless of their morpho-syntactic features (see for example Appendix 1, lines \ref{Guapp25} and \ref{Guapp26}). 
...
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig5.eps}}
...
Based solely on the formal characteristics identified in \refsec{Gusec:recapitulative}, I isolate five clear tokens of \isi{recapitulative linkage} in this text. Two instances of \isi{recapitulative linkage}, the pairs (9--10) and (12--13), are what I will consider ``canonical'' examples. In both cases, the reference clauses (lines 9 and 12) are final clauses with falling \isi{intonation}.  The bridging clauses (lines 10 and 13) are \isi{coordinated} to the following clause with \textit{ro} `and'. The bridging clauses are immediately adjacent to the reference clauses and repeat the lexical content verbatim. Both bridging clauses have rising \isi{intonation} contours.\footnote{A reviewer asked why line 11, which I call a \isi{repetition}, was not taken as the bridging clause of line 9. It is indeed possible to envisage a scenario where line 10 is a false start. The speaker starts the bridging clause line 10, changes her mind, and repeats it as line 11 with added  material.}  

The other three recapitulative linkages appear in the pairs (6--8), (15--18), and (23--24). In the first two linkages, (6--8) and (15--18), the reference and bridging clauses are not immediately adjacent. The \isi{recapitulative linkage} (6--8) shows \isi{addition} and \isi{substitution}. The reference clause (line 6) has three consecutive verbs. The first two are separated from the third verb by the coordinator \textit{ro} `and' in the bridging clause (line 8). The first verb of the reference clause is replaced in the bridging clause by a synonym (i.e., \textit{lai} `take' > \textit{lavi} `take'). The pair (15--18) shows \isi{addition} and \isi{omission} in the bridging clause. The pronoun \textit{nna} `it' is added in the bridging clause, the location, \textit{na apu} `on the fire' present in the reference clause is omitted in the bridging clause. Last, the pair (23--24) also shows \isi{addition}. The bridging clause contains a more complex predicate: \textit{mov} is a phasal predicate \citep[][342]{guerin11}, added to the predicate of the reference clause \textit{rororo}, an ideophone representing the sound of sizzling food. 

With respect to placement, the bridging clauses lines 13 and 24 are surrounded by main line events, i.e., new steps in the recipe. The bridging clauses lines 8 and 18 are preceded by advisory comments on the \isi{supporting line} (lines 7, 16, and 17). They are followed by a new main line event, lines 9 and 19. The reference clause line 9 is preceded by the bridging clause from the previous \isi{recapitulative linkage}. It does not contain a new event per se but an elaboration of the event on the \isi{main event line}, on line 6. The bridging clause line 10 is followed by a \isi{repetition} of itself, line 11, with an added aspectual dimension and continuation \isi{intonation}. 

By looking at the placement of the bridging clauses in the text, we can better deduce their function. The two bridging clauses which appear after material on the \isi{supporting line} (lines 8 and 18) flag a change of orientation, from background to foreground. They bring the \isi{topic} and the audience back onto the \isi{main event line}. On the other hand, the bridging clauses surrounded by main line events (lines 13 and 24) signal that the procedure is continuing. They highlight the \isi{sequentiality} of each step in the recipe and thrust the recipe forward. Recapping one event on the main line (the reference clause) before the next event (in the clause after the bridging clause) ``transform[s] the repeated item from new into given information'' \citep[][224]{brown.2000}.   
...
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig6.eps}}
...
%ex17
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig7.eps}}
...
\begin{figure}[ht]
\fbox{\includegraphics[width=4.8in]{chapters/Figures/guerinFig8.eps}}
...
\glt \sqt{then you squeeze [out the milk] in a cooking pot.} 
\end{exe}
...
\glt \sqt{ you squeeze [out the milk] in a cooking pot and} 
\end{exe}
...
\glt \sqt{then you remove [it from the fire]} 
\end{exe}
...
\glt \sqt{then, you pour it down in a bottle.} 
\end{exe}
...
Sincere thanks to both reviewers whose insightful comments helped improve this chapter, and to my consultants, friends, and families on \ili{Mavea} Island.
